...
DB_USER = "jw3696"
DB_PASSWORD = "OvO_w+l0503"
DB_SERVER = "localhost"
DATABASEURI = "postgresql://"+DB_USER+":"+DB_PASSWORD+"@"+DB_SERVER+"/data"
...
def index():
	...
	cursor = g.conn.execute("WITH PopWine(popwid) AS \
		(SELECT wid \
		FROM UserWishWine WW \
		WHERE wid IN \
			(SELECT wid FROM Review U GROUP BY wid HAVING AVG(U.point) > 80)\
		GROUP BY wid ORDER BY COUNT(uid) DESC LIMIT 10) \
		SELECT winery, grapeType,popwid \
		FROM (PopWine P JOIN Wine W ON P.popwid = W.wid) AS PW \
		JOIN Location L ON PW.lid = L.lid;")

	names = []
	for result in cursor:
		names.append([result['winery'],result['grapetype'],result['popwid']]) 
	cursor.close()

	context = dict(data = names)
	return render_template("index.html", **context)

@app.route('/signup', methods = ['GET','POST']) #revised
def signup():
	if request.method == 'POST':
		...
		try:
			...
			flash("User ID already existed. Please retry.")
			return redirect('/signup')

	return render_template("signup.html") 

@app.route('/login', methods = ['GET','POST']) #revised
def login():
	if request.method == 'POST':
		uid = request.form['uid']
		psw = request.form['psw']
...
def user(uid):
	...
	wished = g.conn.execute('SELECT u.wid, grapetype FROM userwishwine u JOIN wine w ON u.wid=w.wid WHERE uid = %s', (uid,))
	wine = []
	for item in wished:
		winestr = "Wine #%d : %s Wine"%(item['wid'],item['grapetype'])
		wine.append([winestr,item['wid']])
	wished.close()
	if not wine:
...
def wineInfo(wineid, methods=['POST']):
	...
	for item in review_list:
		pointStr = 'Point: ' + str(item['point']) + '/100'
		info=[item['name'],pointStr]
		wine.append(info)
		...
		if item['rid'] in rid:
			...
			myRev.append(wine.index(info))
			print(myRev)
			reviewDict[str(wine.index(info))] = item['rid']
		##################################

		wineInfoString = item['description']
		wine.append(wineInfoString)
		wineInfoString = "-"
		wine.append(wineInfoString)
		index = index + 1
...
@app.route('/search' , methods = ['GET','POST']) #FINISHED # INJECTION CLEARED #revised
def search():
	if request.method == 'POST':
		info = {}
		...
		for item in valid_wine:
			wineID = item['wid']
			wineName = item['grapetype'] + " Wine"
			wineCountry = ' '
			if item['country'] is not None:
				wineCountry = item['country']
			wine.append([wineID,wineName,wineCountry])
